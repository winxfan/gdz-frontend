...
Показывать каждому пользователю персонализированную «зверушку» (аватар + имя) на основе IP-адреса. Для всех пользователей с одинаковым IP — один и тот же аватар и одно и то же имя. Набор животных — 5: **лиса, панда, енот, сова, пингвин**. Аватарки хранятся локально/на CDN как `avatar1.png` … `avatar5.png` (соответствуют порядку выше).
...
* Аватар животного **должен совпадать с именем животного** (лиса → avatar1.png и т.д.).
...
1. `1` — лиса — `avatar1.png` — род: женский
2. `2` — панда — `avatar2.png` — род: женский
3. `3` — енот — `avatar3.png` — род: мужской
4. `4` — сова — `avatar4.png` — род: женский
5. `5` — пингвин — `avatar5.png` — род: мужской
...
5. Сформировать путь к аватару `assets/avatars/avatar{index+1}.png`.
...
export function getAvatarAndNameByIp(ip: string) {
  ...
  const avatarFile = `/assets/avatars/avatar${animal.id}.png`; // индекс 1..5
  ...
  useEffect(() => {
    fetch("/api/avatar-info")
      ...
      .catch(() => {
        ...
        setData({ avatarUrl: "/assets/avatars/avatar1.png", displayName: "Дружелюбная лиса" });
      });
  }, []);
...
* Файлы: `avatar1.png`, `avatar2.png`, `avatar3.png`, `avatar4.png`, `avatar5.png`
...
Ниже — промпты для генерации единого стильного набора (каждый промпт менять только `{ЖИВОТНОЕ}` и `avatar{index}.png` в метаданных при сохранении).
...
- Сохранить файл как: avatar{index}.png
```

### Примеры для каждого животного (вставлять вместо `{ЖИВОТНОЕ}`)

* для `avatar1.png`: `{ЖИВОТНОЕ} = лиса`
* для `avatar2.png`: `{ЖИВОТНОЕ} = панда`
* для `avatar3.png`: `{ЖИВОТНОЕ} = енот`
* для `avatar4.png`: `{ЖИВОТНОЕ} = сова`
* для `avatar5.png`: `{ЖИВОТНОЕ} = пингвин`
...
* Предзагрузить 5 маленьких аватаров на страницу: `<link rel="preload" as="image" href="/assets/avatars/avatar1.png">` и т.д.
...
* Если IP — локальный (например, `127.0.0.1`, `10.x.x.x`, `192.168.x.x`) или невалидный, возвращать предсказуемый fallback (например, `avatar1.png` — лиса), либо вычислять по хэшу client-generated id.
...
* Visual tests:
  ...
  * Перекрестная проверка: avatarX.png соответствует animalRu.
...
* Версионируй аватарки (например `avatar1.v1.png`) если планируешь пересоздавать.

## 18. Пример набора тестовых IP → результатов (для QA)

* `8.8.8.8` → (пример) `avatar3.png`, `Дружелюбный енот`
* `1.2.3.4` → `avatar2.png`, `Тихая панда`
...
1. Подготовить и положить в `public/assets/avatars/` файлы `avatar1.png`…`avatar5.png`.
